import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute } from '@angular/router';
import { RouterTestingModule } from '@angular/router/testing';
import { of } from 'rxjs';

import { DatabaseVersionService } from '../service/database-version.service';

import { DatabaseVersionComponent } from './database-version.component';

describe('DatabaseVersion Management Component', () => {
  let comp: DatabaseVersionComponent;
  let fixture: ComponentFixture<DatabaseVersionComponent>;
  let service: DatabaseVersionService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [
        RouterTestingModule.withRoutes([{ path: 'database-version', component: DatabaseVersionComponent }]),
        HttpClientTestingModule,
      ],
      declarations: [DatabaseVersionComponent],
      providers: [
        {
          provide: ActivatedRoute,
          useValue: {
            data: of({
              defaultSort: 'id,asc',
            }),
            queryParamMap: of(
              jest.requireActual('@angular/router').convertToParamMap({
                page: '1',
                size: '1',
                sort: 'id,desc',
              })
            ),
            snapshot: { queryParams: {} },
          },
        },
      ],
    })
      .overrideTemplate(DatabaseVersionComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(DatabaseVersionComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(DatabaseVersionService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.databaseVersions?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to databaseVersionService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getDatabaseVersionIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getDatabaseVersionIdentifier).toHaveBeenCalledWith(entity);
      expect(id).toBe(entity.id);
    });
  });
});
